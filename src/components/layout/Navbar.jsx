...
import appLinksData from "./AppLinksData";
import { getPhotoUrl, preloadImage } from '../utils/imageUtils';

const PROFILE_BASE_PATH = "https://formafusionmg.ams3.cdn.digitaloceanspaces.com/formafusionmg/img/referents/";
...
const Navbar = () => {
  ...
  const dropdownStates = useMemo(() => [
    { ref: mobileMenuRef, setter: setIsMobileMenuOpen, name: "mobile", toggleButtonSelector: 'button[aria-label="Toggle mobile menu"]' },
    { ref: chiffreDAffaireRef, setter: setIsChiffreDAffaireOpen, name: "chiffreAffaire", toggleButtonSelector: 'button[aria-label="Menu Chiffre d\'affaire"]' },
    { ref: notificationsRef, setter: setIsNotificationsOpen, name: "notifications", toggleButtonSelector: 'button[title="Notifications"]' },
    { ref: appsDropdownRef, setter: setIsAppsDropdownOpen, name: "apps", toggleButtonSelector: 'button[aria-label="Applications"]' },
    { ref: profileDropdownRef, setter: setIsProfileDropdownOpen, name: "profile", toggleButtonSelector: 'button[aria-label="Menu de profil"]' },
  ], []);

  const closeAllDropdownsExcept = useCallback((dropdownToKeepOpen = null) => {
  ...
  useEffect(() => {
    const handleClickOutside = (event) => {
      dropdownStates.forEach(({ ref, setter, toggleButtonSelector }) => {
        if (ref.current && !ref.current.contains(event.target) && !event.target.closest(toggleButtonSelector)) {
          setter(false);
        }
      });
    };
  ...
  useEffect(() => {
    const fetchProfileImage = () => {
      ...
      preloadImage(url).catch(e => console.error("Failed to preload image", e));
    };

    if (isAuthenticated && user) {
      fetchProfileImage();
    } else {
      setProfileImageUrl(getPhotoUrl(null, {
        basePath: PROFILE_BASE_PATH,
        defaultImage: DEFAULT_PROFILE_IMAGE
      }));
    }
  ...
  const isNavLinkActive = useCallback((path) => {
    ...
    return path === "/" ? currentPath === path : currentPath.startsWith(path) && (currentPath.length === path.length || currentPath[path.length] === "/");
  }, [location.pathname]);
            ...
            <div className="flex items-center space-x-4">
              ...
              <div className="relative" ref={appsDropdownRef}>
                <button
                  ...
                  className="cursor-pointer p-2 rounded-full text-slate-600 hover:bg-slate-100 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-purple-500"
                        ...
                        <div className="flex items-center w-full gap-3 mb-4">
                          ...
                          <div className="flex flex-col w-full gap-1">
                            <Link
                              ...
                              target="_blank"
                              rel="noopener noreferrer"
                              className="block px-2 py-1 text-base text-gray-500 transition duration-100 rounded-md hover:bg-gray-100 hover:text-gray-700"
                              onClick={() => closeAllDropdownsExcept()}
                            >
                              Gérer mon profil
                            </Link>
                            ...
                            <button
                              ...
                              className="block w-full text-left px-2 py-1 text-base text-gray-500 transition duration-100 rounded-md hover:bg-gray-100 hover:text-gray-700 cursor-pointer"
