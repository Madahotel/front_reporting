...
const FormationReport = () => {
    ...
    const [displayFormationName, setDisplayFormationName] = useState('Toutes les formations');
    ...
    const calculateItemsPerPage = useCallback(() => {
        if (tableBodyRef.current) {
            ...
            const calculatedItems = Math.max(5, Math.floor(tableBodyHeight / rowHeight)); // Ensure at least 5 items
    ...
    const fetchFormationData = useCallback(async (currentDateRange, currentFormation) => {
        setLoading(true);
        setError(null);

        let daterangePayload;
        if (currentDateRange.range === 'all' || !currentDateRange.startDate || !currentDateRange.endDate) {
            daterangePayload = '01/01/1900 - 12/31/2999';
        } else {
            daterangePayload = `${format(currentDateRange.startDate, 'MM/dd/yyyy')} - ${format(currentDateRange.endDate, 'MM/dd/yyyy')}`;
        }
        ...
        try {
            const response = await api.post('cfp/reporting/filterFormation', payload);
            ...
            if (data.data_filter && data.data_filter.length === 2) {
                ...
                const foundFormation = (data.all_cfp_formation || []).find(f => f.idModule === parseInt(currentFormation));
                setDisplayFormationName(currentFormation === 'all' ? 'Toutes les formations' : (foundFormation?.module_name || currentFormation));
            }

        } catch (error) {
            console.error("Erreur lors de la récupération des données :", error);
            setError("Impossible de charger les données. Veuillez réessayer plus tard. " + (error.response?.data?.message || error.message));
        } finally {
            setLoading(false);
        }
    }, []);
    ...
    const handleFilter = ({ dateRange: newDateRange, formation: newFormation }) => {
    ...
    const getPageNumbers = () => {
        ...
        if (totalPages <= maxPagesToShow) {
            ...
            pageNumbers.push(1);
            if (currentPage > 2 + Math.floor(maxPagesToShow / 2) && currentPage <= totalPages - Math.floor(maxPagesToShow / 2)) {
                pageNumbers.push('...');
                for (let i = currentPage - Math.floor((maxPagesToShow - 3) / 2); i <= currentPage + Math.floor((maxPagesToShow - 3) / 2); i++) {
                    pageNumbers.push(i);
                }
                pageNumbers.push('...');
            } else if (currentPage <= 2 + Math.floor(maxPagesToShow / 2)) { // Near start
            ...
            if (!pageNumbers.includes(totalPages)) { // Ensure last page is always shown if not already
                        ...
                        <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between">
                            <div>
                                ...
                                <div className="text-sm text-gray-500 mt-1">
                                    <span className="font-medium">Dates :</span> {displayDateRange} |
                                    <span className="font-medium ml-2">Formation :</span> {displayFormationName}
                                </div>
                            </div>
                        ...
                        <table className="min-w-full divide-y divide-gray-200">
                            ...
                            <tbody ref={tableBodyRef} className="bg-white divide-y divide-gray-200">
                                ...
                                {!loading && !error && currentData.length === 0 && (
                                    <tr>
                                        <td colSpan="10" className="px-5 py-4 text-center text-sm text-gray-500">
                                            Aucune donnée de formation trouvée pour les filtres actuels ou sur cette page.
                                        </td>
                                    </tr>
                                )}
                                {!loading && !error && currentData.map((data, index) => (
                                    <tr key={index} className="hover:bg-gray-50 transition-colors">
                                        ...
                                        <td className="px-5 py-3 whitespace-nowrap text-sm text-gray-900 text-right">{data.dureeH} H</td>
                                    </tr>
                                ))}
                            </tbody>
                        </table>
