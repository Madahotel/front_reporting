...
const CoursReporting = () => {
        ...
        const fetchAllData = async () => {
            ...
            try {
                ...
                if (!data || !data.all_learner) {
                    throw new Error("Données de l'API mal formées : 'all_learner' manquant.");
                }
                ...
                setError(`Impossible de charger les données. Veuillez réessayer. Détails: ${err.message}`);
            } finally {
                setLoading(false);
            }
        };
    ...
    if (error) {
        return (
            <div className="bg-red-100 border-l-4 border-red-500 text-red-700 p-4 mt-8 text-center" role="alert">
                            ...
                            <ol className="inline-flex items-center space-x-1 md:space-x-3">
                                ...
                                <li aria-current="page">
                                    <div className="flex items-center">
                                        ...
                                        <span className="text-sm font-medium text-gray-500">Reporting par formation</span>
                                    </div>
                                </li>
                            </ol>
                        </nav>
            ...
            <main className="container mx-auto px-4 py-6 md:px-0">
                ...
                >
                    <div className="bg-[#A462A4] hover:bg-[#A462A4b9] px-6 py-4 text-white">
                        <h2 className="text-xl font-bold text-white tracking-wide">Liste des rapports de formation</h2>
                    </div>
                    <div className="p-6">
                        {/* Filter Form */}
                        <motion.form
                        ...
                        >
                            <div className="grid grid-cols-1 md:grid-cols-4 gap-4 p-4 bg-white rounded-lg shadow-sm">
                                <div className="md:col-span-3">
                                    <label htmlFor="formationSelect" className="sr-only">Sélectionner une formation:</label>
                                    <select
                                        id="formationSelect"
                                        name="formation"
                                    ...
                                    >
                                        ...
                                        {formations.filter(f => f.value !== 'all').map((formation) => (
                                            <option
                                                key={formation.value}
                        ...
                        >
                            ...
                            <table className="min-w-full table-auto divide-y divide-gray-200">
                                <thead className="bg-gray-50">
                                    <tr>
                                        <th scope="col" className="px-3 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">Module</th>
                                        <th scope="col" className="px-3 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">Matricule</th>
                                        <th scope="col" className="px-3 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">Nom et Prénom</th>
                                        <th scope="col" className="px-3 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">Fonction</th>
                                        <th scope="col" className="px-3 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">Salle</th>
                                        ...
                                        <th scope="col" className="px-3 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">Entreprise</th>
                                        <th scope="col" className="px-3 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">Dates (Début - Fin)</th>
                                        <th scope="col" className="px-3 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">Durée (H)</th>
                                    </tr>
                                </thead>
                                    ...
                                    {filteredReports.length > 0 ? (
                                        filteredReports.map((report, index) => (
                                            <motion.tr
                                                ...
                                                variants={itemVariants}
                                            >
                                                <td className="px-3 py-2 whitespace-normal text-xs text-gray-900">{report.module_name}</td>
                                                <td className="px-3 py-2 whitespace-normal text-xs text-gray-500">{report.emp_matricule}</td>
                                                <td className="px-3 py-2 whitespace-normal text-xs text-gray-900">{`${report.emp_name} ${report.emp_firstname}`}</td>
                                                <td className="px-3 py-2 whitespace-normal text-xs text-gray-500">{report.emp_fonction}</td>
                                                <td className="px-3 py-2 whitespace-normal text-xs text-gray-500">{`${report.salle_name} (${report.salle_quartier})`}</td>
                                                <td className="px-3 py-2 whitespace-normal text-xs text-gray-500">{report.project_type}</td>
                                                <td className="px-3 py-2 whitespace-normal text-xs">
                                                    <span className={`px-2 inline-flex text-xs leading-5 font-semibold rounded-full ${
                                                        report.project_status === 'Terminé' ? 'bg-green-100 text-green-800' :
                                                        report.project_status === 'En cours' ? 'bg-yellow-100 text-yellow-800' :
                                                        'bg-gray-100 text-gray-800'
                                                    }`}>
                                                        {report.project_status}
                                                    </span>
                                                </td>
                                                <td className="px-3 py-2 whitespace-normal text-xs text-gray-500">{report.etp_name}</td>
                                                <td className="px-3 py-2 whitespace-normal text-xs text-gray-500">{`${report.dateDebut} - ${report.dateFin}`}</td>
                                                <td className="px-3 py-2 whitespace-normal text-xs text-gray-500">{report.dureeH}</td>
                                            </motion.tr>
                                        ))
                                    ) : (
                                        <motion.tr variants={itemVariants}>
                                            <td colSpan="10" className="px-6 py-4 text-center text-sm text-gray-500"> {/* Corrected colSpan to 10 */}
                                                <p className="mb-1">Aucun rapport disponible pour la formation sélectionnée.</p>
